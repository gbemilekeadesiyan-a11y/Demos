...
- Typography: Clarity City for headings and large display text only — not on Google Fonts, so loaded as a stand-in via `next/font/google` (Barlow Condensed, Black weight, as `--font-heading` in `app/fonts.ts`) until real Clarity City font files are available for `next/font/local`. Everything else stays on the existing sans-serif. Don't re-decide this per page.
...
- All pages must be responsive. Design mobile-first — base styles target small screens, with `md:`/`lg:` variants adding desktop layout. Test every new page at 375px before considering it done.
